const uvWhiteLightForm = require("../models/uvWhiteLightForm");
const uvWhiteLightRecordss = require("../models/uvWhiteLightRecords");
...
const uvWhiteLightAuditTrail = require("../models/uvWhiteLightAuditTrail");
...
exports.Insert = async (req, res) => {
  ...
  try {
    ...
    const newForm = await uvWhiteLightForm.create(
      {
        site_id: site_id,
    ...
    if (Array.isArray(UVvisCalibProcessRecords) && UVvisCalibProcessRecords?.length > 0) {
      ...
      await uvWhiteLightRecordss.bulkCreate(formRecords, { transaction });
    ...
    await uvWhiteLightAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
  ...
  try {
    ...
    req.files?.forEach((file) => {
      if (file.fieldname === "initiatorAttachment") {
        ...
        const match = file.fieldname.match(/uvWhiteLightRecords\[(\d+)\]\[supporting_docs\]/);
        if (match) {
    ...
    const form = await uvWhiteLightForm.findOne({ where: { form_id }, transaction });
    ...
    const existingRecords = await uvWhiteLightRecordss.findAll({
      where: { form_id },
      transaction,
    });
    ...
    for (let i = 0; i < uvWhiteLightRecords?.length; i++) {
      const record = uvWhiteLightRecords[i];
      ...
      if (record_id && existingMap[record_id]) {
        await uvWhiteLightRecordss.update(newData, {
          where: { record_id },
          transaction,
        });
        ...
        const created = await uvWhiteLightRecordss.create(newData, { transaction });
    ...
    if (validAuditEntries?.length > 0) {
      await uvWhiteLightAuditTrail.bulkCreate(validAuditEntries, { transaction });
    }
...
exports.deleteAttachment = async (req, res) => {
  ...
  try {
    const record = await uvWhiteLightRecordss.findOne({ where: { record_id } });
    ...
    await uvWhiteLightRecordss.update(
  ...
  uvWhiteLightForm
    .findAll({
      include: [
        ...
        {
          model: User,
          as: "reviewerss11", // Use the consistent alias 'reviewer'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
        {
          model: User,
          as: "approverss11", // Use the consistent alias 'approver'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
      ],
      order: [["form_id", "DESC"]],
...
exports.SendElogForReview = async (req, res) => {
  ...
  try {
    ...
    // Find the form
    const form = await uvWhiteLightForm.findOne({
      where: { form_id },
      transaction,
    });
...
exports.getAuditTrailForAnElog = async (req, res) => {
  try {
    ...
    const auditTrail = await uvWhiteLightAuditTrail.findAll({
...
//         getData = await uvWhiteLightAuditTrail.findAll({
//           where: { form_id: formId },
//           include: {
//             model: User,
//             attributes: ["user_id", "name"],
