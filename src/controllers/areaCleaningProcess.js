...
exports.InsertAreaCleaning = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await AreaCleaningForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      ...
      // await AreaCleaningRecord.bulkCreate(formRecords, { transaction });
    ...
    await AreaCleaningAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
// Edit Area Cleaning elog details
  exports.EditAreaCleaning = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      req.files?.forEach((file) => {
        ...
        //  else if (file.fieldname.startsWith("AreaCleaningRecords[")) {
        //   const match = file.fieldname.match(
        //     /AreaCleaningRecords\[(\d+)\]\[supporting_docs\]/
        //   );
        //   if (match) {
        //     const index = match[1];
        //     supportingDocs[index] = file;
      ...
      const form = await AreaCleaningForm.findOne({
      ...
      // if (
      //   Array.isArray(AreaCleaningRecords) &&
      //   AreaCleaningRecords.length > 0
      // ) {
      //   const existingRecords = await AreaCleaningRecord.findAll({
      //     where: { form_id: form_id },
      //     raw: true,
      ...
      //     AreaCleaningRecords.sort(
      //       (a, b) => parseInt(a.record_id) - parseInt(b.record_id)
      //     );
      //     const newRecord = AreaCleaningRecords[index];
      //     if (newRecord) {
      //       const recordFields = {
      ...
      //   if (AreaCleaningRecords.length > existingRecords.length) {
      //     for (
      //       let i = existingRecords.length;
      //       i < AreaCleaningRecords.length;
      //       i++
      //     ) {
      //       const newRecord = AreaCleaningRecords[i];
      //       const recordFields = {
      ...
      //   await AreaCleaningRecord.destroy({
      //     where: { form_id: form_id },
      //     transaction,
      //   });

      //   // Create new records
      //   const formRecords = AreaCleaningRecords.map((record, index) => ({
      //     form_id: form_id,
      //     unique_id: record?.unique_id,
      //     time: record?.time,
      //     date: record?.date,
  ...
  // Update / Create Temperature Records (NO DELETE)
        ...
        if (Array.isArray(AreaCleaningRecords) && AreaCleaningRecords.length > 0) {

          for (const record of AreaCleaningRecords) {

            if (record.record_id) {
              // UPDATE existing row
              await AreaCleaningRecord.update(
                {
                  unique_id: record?.unique_id,
                  time: record?.time,
                  date: record?.date,
                  ...
                  cleaningAgent: record?.cleaningAgent,
                  disinfectantUsed: record?.disinfectantUsed,
                  ceiling: record?.ceiling,
                  lightFixtures: record?.lightFixtures,
                  walls: record?.walls,
                  riserGrills: record?.riserGrills,
                  doorCloser: record?.doorCloser,
                  electricFixtures: record?.electricFixtures,
              ...
              // CREATE only new row
              await AreaCleaningRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  time: record?.time,
...
exports.getAuditTrailForAnElog = async (req, res) => {
  try {
    ...
    const auditTrail = await AreaCleaningAuditTrail.findAll({
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await AreaCleaningForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("ac_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
if (!form_id) {
  return res.status(400).json({ error: true, message: "Form Id Required" });
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveACReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.AreaCleaningRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      time: record?.time || "",
      date: record?.date || "",
      differential_pressure: record?.differential_pressure || "",
