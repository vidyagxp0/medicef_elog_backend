...
exports.InsertareaFogging = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await areaFoggingForm.create(
      {
    ...
    //   // await areaFoggingRecord.bulkCreate(formRecords, { transaction });
    ...
    await areaFoggingAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
// edit differential pressure elog details
  exports.EditareaFogging = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      req.files?.forEach((file) => {
        ...
        //  else if (file.fieldname.startsWith("areaFoggingRecords[")) {
        //   const match = file.fieldname.match(
        //     /areaFoggingRecords\[(\d+)\]\[supporting_docs\]/
        //   );
        //   if (match) {
        //     const index = match[1];
        //     supportingDocs[index] = file;
      ...
      const form = await areaFoggingForm.findOne({
  ...
  // Update / Create Temperature Records (NO DELETE)
        if (Array.isArray(areaFoggingRecords) && areaFoggingRecords.length > 0) {

          for (const record of areaFoggingRecords) {

            if (record.record_id) {
              // UPDATE existing row
              await areaFoggingRecord.update(
                {
                  unique_id: record?.unique_id,
              ...
              // CREATE only new row
              await areaFoggingRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  dateOfFogging: record?.dateOfFogging,
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await areaFoggingForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.areaFoggingRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      dateOfFogging: record?.dateOfFogging || "",
      foggingDueOn: record?.foggingDueOn || null,
      foggingTimeStart: record?.foggingTimeStart || "",
