...
exports.InsertfilterCleaning = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await filterCleaningForm.create(
      {
...
// edit differential pressure elog details
  exports.EditfilterCleaning = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      req.files?.forEach((file) => {
        ...
        //  else if (file.fieldname.startsWith("filterCleaningRecords[")) {
        //   const match = file.fieldname.match(
        //     /filterCleaningRecords\[(\d+)\]\[supporting_docs\]/
        //   );
        //   if (match) {
        //     const index = match[1];
        //     supportingDocs[index] = file;
      ...
      const form = await filterCleaningForm.findOne({
  ...
  // Update / Create Temperature Records (NO DELETE)
        if (Array.isArray(filterCleaningRecords) && filterCleaningRecords.length > 0) {

          for (const record of filterCleaningRecords) {
            ...
            if (record.record_id) {
              ...
              // CREATE only new row
              await filterCleaningRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  date: record?.date,
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await filterCleaningForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
