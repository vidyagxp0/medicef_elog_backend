...
exports.InsertDailyCalibration = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await DailyCalibrationForm.create(
      {
        ...
        make: make,
        modelNo: modelNo,
        calibrationDoneOn: calibrationDoneOn,
    ...
    await DailyCalibrationAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditDailyCalibration = async (req, res) => {
  ...
  try {
    ...
    const form = await DailyCalibrationForm.findOne({
    ...
    if (
      Array.isArray(DailyCalibrationRecords) &&
      DailyCalibrationRecords.length > 0
    ) {
      for (const record of DailyCalibrationRecords) {
        if (record.record_id) {
          // UPDATE existing row
          ...
          // CREATE only new row
          await DailyCalibrationRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              dateTime: record?.dateTime,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("da_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await DailyCalibrationForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("da_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveDAReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.DailyCalibrationRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
