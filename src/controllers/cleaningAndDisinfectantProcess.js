...
exports.InsertCleaningAndDisinfectant = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await CleaningAndDisinfectantForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        date: record?.date,
        name_of_solution: record?.name_of_solution,
        concentration_of_solution: record?.concentration_of_solution,
        volume_of_disinfectant: record?.volume_of_disinfectant,
        volume_of_purified: record?.volume_of_purified,
        prepared_by: record?.prepared_by,
        total_volume_of_solution: record?.total_volume_of_solution,
        quantity: record?.quantity,
        department_area: record?.department_area,
        prepared_date: record?.prepared_date,
        remaining_qty_destroyed: record?.remaining_qty_destroyed,
    ...
    await CleaningAndDisinfectantAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditCleaningAndDisinfectant = async (req, res) => {
  ...
  const {
    ...
    reviewer_id,
    approver_id,
    CleaningAndDisinfectantRecords,
    esignInput,
    password,
    initiatorComment,
    additionalInfo,
  ...
  try {
    ...
    const form = await CleaningAndDisinfectantForm.findOne({
    ...
    if (
      Array.isArray(CleaningAndDisinfectantRecords) &&
      CleaningAndDisinfectantRecords.length > 0
    ) {
      for (const record of CleaningAndDisinfectantRecords) {
        if (record.record_id) {
          // UPDATE existing row
          await CleaningAndDisinfectantRecord.update(
            {
              unique_id: record?.unique_id,
              date: record?.date,
              name_of_solution: record?.name_of_solution,
          ...
          // CREATE only new row
          await CleaningAndDisinfectantRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              date: record?.date,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("cd_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await CleaningAndDisinfectantForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("cd_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    const formData = await CleaningAndDisinfectantForm.findOne({
      ...
      include: [
        {
          model: CleaningAndDisinfectantRecord,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveCDReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.CleaningAndDisinfectantRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
