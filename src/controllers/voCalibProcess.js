const voCalibForm = require("../models/voCalibForm");
const voCalibRecordss = require("../models/voCalibRecords");
...
const voCalibAuditTrail = require("../models/voCalibAuditTrail");
...
exports.Insert = async (req, res) => {
  ...
  try {
    ...
    req.files?.forEach((file) => {
      ...
      } else if (file.fieldname.startsWith("voCalibRecords[")) {
        // Extract the index from the fieldname
        const match = file.fieldname.match(
          /voCalibRecords\[(\d+)\]\[supporting_docs\]/
        );
        if (match) {
          const index = match[1];
    ...
    const newForm = await voCalibForm.create(
      {
        site_id: site_id,
    ...
    if (Array.isArray(voCalibRecords) && voCalibRecords?.length > 0) {
      const formRecords = voCalibRecords.map((record, index) => ({
        form_id: newForm?.form_id,
        date: record?.date,
      ...
      await voCalibRecordss.bulkCreate(formRecords, { transaction });
    ...
    await voCalibAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
  ...
  try {
    ...
    req.files?.forEach((file) => {
      if (file.fieldname === "initiatorAttachment") {
        ...
        const match = file.fieldname.match(/voCalibRecords\[(\d+)\]\[supporting_docs\]/);
        if (match) {
    ...
    const form = await voCalibForm.findOne({ where: { form_id }, transaction });
    ...
    const existingRecords = await voCalibRecordss.findAll({
      where: { form_id },
      transaction,
    });
    ...
    for (let i = 0; i < voCalibRecords?.length; i++) {
      const record = voCalibRecords[i];
      ...
      if (record_id && existingMap[record_id]) {
        await voCalibRecordss.update(newData, {
          where: { record_id },
          transaction,
        });
        ...
        const created = await voCalibRecordss.create(newData, { transaction });
    ...
    if (validAuditEntries?.length > 0) {
      await voCalibAuditTrail.bulkCreate(validAuditEntries, { transaction });
    }
...
exports.deleteAttachment = async (req, res) => {
  ...
  try {
    const record = await voCalibRecordss.findOne({ where: { record_id } });
  ...
  voCalibForm
    .findAll({
      include: [
        ...
        {
          model: User,
          as: "reviewerss12", // Use the consistent alias 'reviewer'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
        {
          model: User,
          as: "approverss12", // Use the consistent alias 'approver'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
      ],
      order: [["form_id", "DESC"]],
...
exports.SendElogForReview = async (req, res) => {
  ...
  try {
    ...
    // Find the form
    const form = await voCalibForm.findOne({
      where: { form_id },
      transaction,
    });
...
exports.getAuditTrailForAnElog = async (req, res) => {
  try {
    ...
    const auditTrail = await voCalibAuditTrail.findAll({
...
//     switch (type) {
//       case "voCalibAuditTrail":
//         getData = await voCalibAuditTrail.findAll({
//           where: { form_id: formId },
//           include: {
//             model: User,
//             attributes: ["user_id", "name"],
