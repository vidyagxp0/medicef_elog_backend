...
exports.InsertDispensing = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await DispensingForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        dispensingStartTime: record?.dispensingStartTime,
        dispensingEndTime: record?.dispensingEndTime,
        dispensingDoneBy: record?.dispensingDoneBy,
        dispensingCheckedBy: record?.dispensingCheckedBy,
        verified_by: record?.verified_by,
        cleaningStartTime: record?.cleaningStartTime,
        cleaningEndTime: record?.cleaningEndTime,
        cleaningDoneBy: record?.cleaningDoneBy,
        cleaningVerifiedBy: record?.cleaningVerifiedBy,
    ...
    await DispensingAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditDispensing = async (req, res) => {
  ...
  try {
    ...
    const form = await DispensingForm.findOne({
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("dr_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await DispensingForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("dr_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveDRReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.DispensingRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
