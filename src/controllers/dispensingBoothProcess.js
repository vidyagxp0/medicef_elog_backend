...
exports.InsertDispensingBooth = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await DispensingBoothForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        date: record?.date,
        rlaf_start_time: record?.rlaf_start_time,
        pre_filter: record?.pre_filter,
        intermediate_filter: record?.intermediate_filter,
        hepa_filter: record?.hepa_filter,
        product: record?.product,
        batch_no: record?.batch_no,
        batch_size: record?.batch_size,
        dispensing_start_time: record?.dispensing_start_time,
        dispensing_end_time: record?.dispensing_end_time,
        rlaf_end_time: record?.rlaf_end_time,
        dispensing_doneby: record?.dispensing_doneby,
        dispensing_verifiedby: record?.dispensing_verifiedby,
        dbc_start_time: record?.dbc_start_time,
        dbc_end_time: record?.dbc_end_time,
        dbc_doneby: record?.dbc_doneby,
        dbc_verifiedby: record?.dbc_verifiedby,
    ...
    await DispensingBoothAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditDispensingBooth = async (req, res) => {
  ...
  try {
    ...
    const form = await DispensingBoothForm.findOne({
    ...
    if (
      Array.isArray(DispensingBoothRecords) &&
      DispensingBoothRecords.length > 0
    ) {
      for (const record of DispensingBoothRecords) {
        if (record.record_id) {
          // UPDATE existing row
          await DispensingBoothRecord.update(
            {
              unique_id: record?.unique_id,
              date: record?.date,
              rlaf_start_time: record?.rlaf_start_time,
              ...
              dispensing_doneby: record?.dispensing_doneby,
              dispensing_verifiedby: record?.dispensing_verifiedby,
              type_of_cleaning: record?.type_of_cleaning,
              dbc_start_time: record?.dbc_start_time,
              dbc_end_time: record?.dbc_end_time,
              dbc_doneby: record?.dbc_doneby,
              dbc_verifiedby: record?.dbc_verifiedby,
          ...
          // CREATE only new row
          await DispensingBoothRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              date: record?.date,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("db_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await DispensingBoothForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("db_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveDBReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.DispensingBoothRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
