...
exports.InsertfoggingSolution = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
...
// edit differential pressure elog details
  exports.EditfoggingSolution = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      req.files?.forEach((file) => {
        ...
        //  else if (file.fieldname.startsWith("foggingSolutionRecords[")) {
        //   const match = file.fieldname.match(
        //     /foggingSolutionRecords\[(\d+)\]\[supporting_docs\]/
        //   );
        //   if (match) {
        //     const index = match[1];
        //     supportingDocs[index] = file;
      ...
      const form = await foggingSolutionForm.findOne({
  ...
  // Update / Create Temperature Records (NO DELETE)
        if (Array.isArray(foggingSolutionRecords) && foggingSolutionRecords.length > 0) {

          for (const record of foggingSolutionRecords) {

            if (record.record_id) {
              // UPDATE existing row
              await foggingSolutionRecord.update(
                {
                  unique_id: record?.unique_id,
                  date: record?.date,
                  time: record?.time,
                  virosilTaken: record?.virosilTaken,
                  waterTaken: record?.waterTaken,
                  totalSolution: record?.totalSolution,
              ...
              // CREATE only new row
              await foggingSolutionRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  date: record?.date,
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await foggingSolutionForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("fs_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
if (!form_id) {
  return res.status(400).json({ error: true, message: "Form Id Required" });
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveFSReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
