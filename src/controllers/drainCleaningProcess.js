...
exports.InsertDrainCleaning = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        date: record?.date,
        time: record?.time,
        cleaning_agent: record?.cleaning_agent,
        disinfectant_used: record?.disinfectant_used,
        sanitizer_used: record?.sanitizer_used,
    ...
    await DrainCleaningAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditDrainCleaning = async (req, res) => {
  ...
  try {
    ...
    const form = await DrainCleaningForm.findOne({
    ...
    if (
      ...
      for (const record of DrainCleaningRecords) {
        if (record.record_id) {
          ...
          // CREATE only new row
          await DrainCleaningRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              time: record?.time,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("dc_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("dc_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.DrainCleaningRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
