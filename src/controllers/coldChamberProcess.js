...
exports.InsertColdChamber = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await ColdChamberForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        outer_surface: record?.outer_surface,
        inner_door_surface: record?.inner_door_surface,
        walls: record?.walls,
        ceiling: record?.ceiling,
        light_fixtures: record?.light_fixtures,
        floor: record?.floor,
    ...
    await ColdChamberAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditColdChamber = async (req, res) => {
  ...
  try {
    ...
    const form = await ColdChamberForm.findOne({
    ...
    if (
      Array.isArray(ColdChamberRecords) &&
      ColdChamberRecords.length > 0
    ) {
      for (const record of ColdChamberRecords) {
        if (record.record_id) {
          // UPDATE existing row
          await ColdChamberRecord.update(
            {
              unique_id: record?.unique_id,
              date: record?.date,
              time_from: record?.time_from,
          ...
          // CREATE only new row
          await ColdChamberRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              date: record?.date,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("cc_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await ColdChamberForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("cc_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveCCReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.ColdChamberRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
