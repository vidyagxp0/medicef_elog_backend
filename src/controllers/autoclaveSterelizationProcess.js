...
exports.InsertAutoclaveSterelization = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await AutoclaveSterelizationForm.create(
      {
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      const formRecords = FormRecordsArray.map((record, index) => ({
        ...
        autoclave_load_no: record?.autoclave_load_no,
        cycle_no: record?.cycle_no,
        cycle_hold_time: record?.cycle_hold_time,
        autoclave_start_time: record?.autoclave_start_time,
        autoclave_end_time: record?.autoclave_end_time,
        sterilization_hold_start_time: record?.sterilization_hold_start_time,
        sterilization_hold_end_time: record?.sterilization_hold_end_time,
    ...
    await AutoclaveSterelizationAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditAutoclaveSterelization = async (req, res) => {
  ...
  try {
    ...
    const form = await AutoclaveSterelizationForm.findOne({
    ...
    if (
      Array.isArray(AutoclaveSterelizationRecords) &&
      AutoclaveSterelizationRecords.length > 0
    ) {
      for (const record of AutoclaveSterelizationRecords) {
        if (record.record_id) {
          // UPDATE existing row
          await AutoclaveSterelizationRecord.update(
            {
              unique_id: record?.unique_id,
              date: record?.date,
              material_name: record?.material_name,
          ...
          // CREATE only new row
          await AutoclaveSterelizationRecord.create(
            {
              form_id: form_id,
              unique_id: record?.unique_id,
              date: record?.date,
...
exports.generateReport = async (req, res) => {
  try {
    ...
    const html = await new Promise((resolve, reject) => {
      res.render("as_report", { reportData }, (err, html) => {
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await AutoclaveSterelizationForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("as_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.effetiveChatByPdf = async (req, res) => {
  try {
    ...
    const formData = await AutoclaveSterelizationForm.findOne({
      ...
      include: [
        {
          model: AutoclaveSterelizationRecord,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveASReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.AutoclaveSterelizationRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      remarks: record?.remarks || "",
      checked_by: record?.checked_by || "",
