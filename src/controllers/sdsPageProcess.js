...
const sdsPageAuditTrail = require("../models/sdsPageAuditTrail");
...
exports.InsertsdsPage = async (req, res) => {
  const {
    site_id,
    description,
    department,
  ...
  try {
    ...
    const newForm = await sdsPageForm.create(
      {
        site_id: site_id,
    ...
    await sdsPageAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
exports.EditsdsPage = async (req, res) => {
  const {
    form_id,
    site_id,
    description,
  ...
  try {
    ...
    const form = await sdsPageForm.findOne({ where: { form_id }, transaction });
    ...
    if (validAuditEntries?.length > 0) {
      await sdsPageAuditTrail.bulkCreate(validAuditEntries, { transaction });
    }
...
//deleting attachment
exports.deletesdsPageAttachment = async (req, res) => {
...
exports.GetsdsPageElog = async (req, res) => {
...
exports.GetAllsdsPageElog = async (req, res) => {
  sdsPageForm
    .findAll({
      include: [
        {
        ...
        {
          model: User,
          as: "reviewerss9", // Use the consistent alias 'reviewer'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
        {
          model: User,
          as: "approverss9", // Use the consistent alias 'approver'
          attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
        },
      ],
      order: [["form_id", "DESC"]],
...
exports.SendElogForReview = async (req, res) => {
  ...
  try {
    ...
    // Find the form
    const form = await sdsPageForm.findOne({
      where: { form_id },
      transaction,
    });
...
exports.getAuditTrailForAnElog = async (req, res) => {
  try {
    ...
    const auditTrail = await sdsPageAuditTrail.findAll({
...
//         getData = await sdsPageAuditTrail.findAll({
//           where: { form_id: formId },
//           include: {
//             model: User,
//             attributes: ["user_id", "name"],
