...
exports.InsertInstrumentUsage = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await instrumentUsageForm.create(
      {
    ...
    //   // await instrumentUsageRecord.bulkCreate(formRecords, { transaction });
    ...
    await instrumentUsageAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
// edit differential pressure elog details
  exports.EditInstrumentUsage = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      req.files?.forEach((file) => {
        ...
        //  else if (file.fieldname.startsWith("instrumentUsageRecords[")) {
        //   const match = file.fieldname.match(
        //     /instrumentUsageRecords\[(\d+)\]\[supporting_docs\]/
        //   );
        //   if (match) {
        //     const index = match[1];
        //     supportingDocs[index] = file;
      ...
      const form = await instrumentUsageForm.findOne({
  ...
  // Update / Create Temperature Records (NO DELETE)
        if (Array.isArray(instrumentUsageRecords) && instrumentUsageRecords.length > 0) {

          for (const record of instrumentUsageRecords) {

            if (record.record_id) {
              // UPDATE existing row
              await instrumentUsageRecord.update(
                {
                  unique_id: record?.unique_id,
                  date: record?.date,
                  time: record?.time,
              ...
              // CREATE only new row
              await instrumentUsageRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  date: record?.date,
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await instrumentUsageForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.instrumentUsageRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      time: record?.time || "",
      date: record?.date || "",
      differential_pressure: record?.differential_pressure || "",
