...
exports.InsertmediaConsumption = async (req, res) => {
  const {
    department_id,
    process_id,
    description,
  ...
  try {
    ...
    const newForm = await mediaConsumptionForm.create(
      {
    ...
    await mediaConsumptionAuditTrail.bulkCreate(auditTrailEntries, {
      transaction,
    });
...
// edit differential pressure elog details
  exports.EditmediaConsumption = async (req, res) => {
    const {
      department_id,
      description,
      departmentName,
    ...
    try {
      ...
      const form = await mediaConsumptionForm.findOne({
  ...
  // Update / Create Temperature Records (NO DELETE)
        if (Array.isArray(mediaConsumptionRecords) && mediaConsumptionRecords.length > 0) {

          for (const record of mediaConsumptionRecords) {

            if (record.record_id) {
              // UPDATE existing row
              await mediaConsumptionRecord.update(
                {
                  unique_id: record?.unique_id,
                  dateOfUse: record?.dateOfUse,
                  containerNo: record?.containerNo,
                  mediaQty: record?.mediaQty,
                  mediaQtyWithdrawn: record?.mediaQtyWithdrawn,
                  mediaBalanceQty: record?.mediaBalanceQty,
                  autoclaveMedia: record?.autoclaveMedia,
                  area: record?.area,
              ...
              // CREATE only new row
              await mediaConsumptionRecord.create(
                {
                  form_id: form_id,
                  unique_id: record?.unique_id,
                  dateOfUse: record?.dateOfUse,
...
exports.chatByPdf = async (req, res) => {
  try {
    const { form_id } = req.params;
    const formData = await mediaConsumptionForm.findOne({
      where: { form_id },
      include: [
        {
          model: Process,
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("mc_report", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
...
if (!form_id) {
  return res.status(400).json({ error: true, message: "Form Id Required" });
    ...
    // Render HTML using EJS template
    const html = await new Promise((resolve, reject) => {
      req.app.render("effectiveMCReport", { reportData }, (err, html) => {
        if (err) return reject(err);
        resolve(html);
      });
    });
    ...
    const filePath = path.resolve("public", `IU_Elog_Report_${uniqueId}.pdf`);
    fs.writeFileSync(filePath, pdf);

    res.status(200).json({ filename: `IU_Elog_Report_${uniqueId}.pdf` });
  } catch (error) {
    console.error("Error generating PDF:", error);
    return res
      .status(500)
...
exports.blankReport = async (req, res) => {
  try {
    ...
    const data = reportData?.mediaConsumptionRecords?.map((record) => ({
      unique_id: record?.unique_id || "",
      dateOfUse: record?.dateOfUse || "",
      containerNo: record?.containerNo || "",
      mediaQty: record?.mediaQty || 0,
      mediaQtyWithdrawn: record?.mediaQtyWithdrawn || 0,
      mediaBalanceQty: record?.mediaBalanceQty || 0,
      autoclaveMedia: record?.autoclaveMedia || 0,
      remarks: record?.remarks || "",
      done_by: record?.done_by || "",
